<template>
  <div>
    ...
    <!-- Các phần tử khác trong template -->

    <!-- Phần hiển thị danh sách đơn hàng hoặc thông báo -->
    <v-row>
      <v-col cols="12">
        ...
        <template v-if="orders.length > 0">
          <v-card class="pa-2" style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <v-table>
              ...
              <tbody>
                <!-- Lặp qua danh sách đơn hàng và hiển thị thông tin -->
...
export default {
  ...
  computed: {
  filteredOrders() {
    return this.orders.filter(order => order.status === this.tabs[this.selectedTab]);
  }
},
  ...
  this.updateOrders();
    ...
    fetchDataFromBackend() {
      axios.get("https://localhost:44367/api/OrderDetail/GetOrderDetail?newUserID=1")
        .then(response => {
          this.processBackendData(response.data);
          this.orders=response.data
          console.log("oke",this.orders)
